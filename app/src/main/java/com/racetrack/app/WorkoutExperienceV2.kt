...
private const val WALK_STRIDE_FACTOR = 0.414f

private fun estimatedStrideMeters(profile: ProfileStore): Float =
    (profile.heightCm.coerceIn(100f, 230f) / 100f * WALK_STRIDE_FACTOR)

private fun estimatedDistanceMeters(steps: Int, profile: ProfileStore): Float =
    steps.coerceAtLeast(0) * estimatedStrideMeters(profile)

private fun estimatedStepCalories(steps: Int, profile: ProfileStore): Float {
    val weight = profile.weightKg.coerceIn(30f, 250f)
    return steps.coerceAtLeast(0) * 0.04f * (weight / 70f)
}

@Composable
fun WorkoutExperienceHome(
    steps: Int,
    ...
    val distance = estimatedDistanceMeters(steps, profile)
    val calories = estimatedStepCalories(steps, profile)
    ...
    LazyColumn(
        ...
        item {
            Column {
                Text(
                    if (profile.name.isBlank()) "Good morning" else "Good morning, ${profile.name}",
                    color = Color.White,
                ...
                Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.padding(22.dp)) {
                    Box(Modifier.size(230.dp), contentAlignment = Alignment.Center) {
                        Canvas(Modifier.fillMaxSize()) {
                            drawArc(
                                Color(0xFF303030), -90f, 360f, false,
                                style = Stroke(18.dp.toPx(), cap = StrokeCap.Round)
                            )
                            if (progress > 0f) {
                                drawArc(
                                    Brush.sweepGradient(listOf(Green, Cyan, Green)),
        ...
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp), modifier = Modifier.fillMaxWidth()) {
                V2MetricCard("🔥", "Calories", "${calories.roundToInt()} kcal", Modifier.weight(1f))
                V2MetricCard("📏", "Distance", "%.2f km".format(distance / 1000f), Modifier.weight(1f))
            }
        }
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp), modifier = Modifier.fillMaxWidth()) {
                V2MetricCard("⏱", "Active Time", "$activeMinutes min", Modifier.weight(1f))
                V2MetricCard("👟", "Workouts", today.size.toString(), Modifier.weight(1f))
            }
        }
        item {
            Button(
...
@Composable
private fun V2MetricCard(icon: String, title: String, value: String, modifier: Modifier) {
    Card(
...
@Composable
fun WorkoutExperienceLive(
    profile: ProfileStore,
    tracker: StepTracker,
    onFinish: (steps: Int, durationSeconds: Long, distanceMeters: Float, calories: Float, activity: String) -> Unit
) {
    ...
    val latestCalories by rememberUpdatedState(workoutCalories(profile, activity, elapsed, distance))
    ...
    LaunchedEffect(Unit) {
        ...
        while (true) {
            ...
            if (running) {
                elapsed++
                locationTracker.addElapsedSeconds(elapsed)
            }
        }
    }
    ...
    fun finishOnce() {
        if (finished.compareAndSet(false, true)) {
            onFinish(latestSteps, latestElapsed, latestDistance, latestCalories, latestActivity)
        }
    }

    BackHandler { finishOnce() }

    val calories = workoutCalories(profile, activity, elapsed, distance)
    val paceSecondsPerKm = if (distance >= 20f && elapsed > 0L) elapsed.toFloat() * 1000f / distance else 0f
    val accuracyLabel = if (accuracy > 0f) "GPS ±${accuracy.roundToInt()} m" else "Waiting for GPS"

    Column(Modifier.fillMaxSize().background(Charcoal)) {
        Box(Modifier.fillMaxWidth().weight(1f)) {
            WorkoutRouteMap(route)
            Card(
                ...
                Text(
                    accuracyLabel,
                    color = if (accuracy in 0.1f..20f) Green else Muted,
                    fontSize = 11.sp,
        ...
        Card(
            ...
            Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    V2LiveMetric("Steps", tracker.steps.toString())
                    V2LiveMetric("Distance", "%.2f km".format(distance / 1000f))
                    V2LiveMetric("Pace", if (paceSecondsPerKm > 0f) formatPace(paceSecondsPerKm) else "—")
                    V2LiveMetric("Calories", "${calories.roundToInt()} kcal")
                }
...
@Composable
private fun V2LiveMetric(label: String, value: String) {
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
...
private fun workoutCalories(profile: ProfileStore, activity: String, seconds: Long, distanceMeters: Float): Float {
    if (seconds <= 0L) return 0f
    val weight = profile.weightKg.coerceIn(30f, 250f)
    val met = if (activity == "Run") 7.0 else 3.5
    val minutes = seconds / 60.0
    val metCalories = met * 3.5 * weight / 200.0 * minutes
    return max(metCalories, estimatedStepCalories(0, profile).toDouble()).toFloat()
}

private fun formatPace(secondsPerKm: Float): String {
    val totalSeconds = secondsPerKm.roundToInt().coerceIn(1, 99 * 60 + 59)
    return "%d:%02d /km".format(totalSeconds / 60, totalSeconds % 60)
}

@Composable
fun WorkoutExperienceProgress(
    stepStore: StepDataStore,
    workoutStore: WorkoutStore,
    profile: ProfileStore
) {
    ...
    val rangeStart = startOfDay() - (days - 1L) * DAY_MS
    val sessions = workoutStore.forRange(rangeStart, startOfDay() + DAY_MS)
    val distance = estimatedDistanceMeters(totalSteps, profile)
    val calories = estimatedStepCalories(totalSteps, profile)
    val activeMinutes = sessions.sumOf { it.durationSeconds } / 60L
    val points = if (selected == 0) values else values.takeLast(minOf(12, values.size))
    ...
    LazyColumn(
        ...
        item {
            Row(
                Modifier.fillMaxWidth().background(Card, RoundedCornerShape(18.dp)).padding(4.dp),
        ...
        item {
            Card(shape = RoundedCornerShape(22.dp), colors = CardDefaults.cardColors(containerColor = Card)) {
                Column(Modifier.padding(18.dp)) {
                    ...
                    Text("%,d".format(totalSteps), color = Color.White, fontSize = 30.sp, fontWeight = FontWeight.Black)
        ...
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp), modifier = Modifier.fillMaxWidth()) {
                V2MetricCard("⏱", "Active Time", "$activeMinutes min", Modifier.weight(1f))
                V2MetricCard("👟", "Workouts", sessions.size.toString(), Modifier.weight(1f))
            }
        }
        item {
            Card(shape = RoundedCornerShape(20.dp), colors = CardDefaults.cardColors(containerColor = Card)) {
                Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    if (sessions.isEmpty()) {
                        Text("No Walk / Run sessions recorded in this period.", color = Muted, fontSize = 13.sp)
                    } else {
        ...
        values.forEach { value ->
            Box(
                Modifier.width(18.dp)
                    .height((120f * value / maxValue).coerceAtLeast(4f).dp)
...
@Composable
private fun WorkoutRouteMap(route: List<Location>) {
    ...
    AndroidView(
        factory = { context ->
            WebView(context).apply {
                ...
                loadDataWithBaseURL("https://www.openstreetmap.org/", workoutMapHtml(), "text/html", "UTF-8", null)
            }
        },
        update = { web ->
...
private fun workoutMapHtml() = """
<!doctype html>
<html>
<head>
<meta name='viewport' content='width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no'>
<link rel='stylesheet' href='https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'/>
<style>html,body,#map{height:100%;width:100%;margin:0;background:#121212}</style>
</head>
<body><div id='map'></div>
<script src='https://unpkg.com/leaflet@1.9.4/dist/leaflet.js'></script>
<script>
let map=null,line=null,marker=null,mapReady=false;
function boot(){
  if(typeof L==='undefined'){setTimeout(boot,250);return;}
  map=L.map('map',{zoomControl:true,attributionControl:true}).setView([20.5937,78.9629],5);
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap contributors'}).addTo(map);
  line=L.polyline([],{color:'#00E676',weight:6,lineCap:'round',lineJoin:'round'}).addTo(map);
  mapReady=true;
  setTimeout(function(){map.invalidateSize();},150);
}
window.updateRoute=function(points){
  if(!mapReady || !points || !points.length)return;
  line.setLatLngs(points);
  if(marker)map.removeLayer(marker);
  marker=L.circleMarker(points[points.length-1],{radius:7,color:'#00B0FF',weight:3,fillColor:'#00E676',fillOpacity:1}).addTo(map);
  if(points.length===1){map.setView(points[0],17);}
  else{map.fitBounds(line.getBounds(),{padding:[30,30],maxZoom:17});}
